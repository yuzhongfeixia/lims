...
//        ' ', { id: mep + 'btnPrint', text: gpersist.STR_BTN_PRINT, iconCls: 'icon-print', handler: me.OnPrint, scope: me },
  ...
  OnShow:function(){
    var me = this;
    var mep = me.tranPrefix;
    var record = tools.OnGridLoad(me.plGrid, '请选择要查看的数据！');
    if (!record) 
      return;
  ...
  OnShowGetHtml:function(item){
    ...
    var mep = me.tranPrefix;
    var height = 60;
    var record = tools.JsonGet('QuanGetQuanMonitWork.do?qmw.tranid='+item.tranid);
    ...
    html += '<tr class="infotrheader"><td colspan="10" align="center" width="100%" style="height: 40px;font-size:20px"><b>监督工作记录表</b></td></tr>';
    html+='<tr class="infotrheader" border=0><td colspan="5" align="left" width="50%" >NO：</td><td colspan="5" align="right" width="50%" > QCR 5-7-1</td></tr>';
    html += '</table>';
    html += '<table cellspacing="0" cellpadding="0"  border="1" align="center" height="20%" style="font-size:12px;" width="90%">';
    html += '<tr class="infotr"><td class="infoc infoleft" colspan=2 align="left" width="5%" style="height:40px">' +
        '质量监督员    ' +alms.GetItemData(record.monitusername) + '  于     ' + alms.GetItemDateData(record.monitstart) + '  至     ' +
        alms.GetItemDateData(record.monitend)+ '  对       ' +  alms.GetItemData(record.trialusername) + '  检测的    ' +
        alms.GetItemData(record.sampleid) +'（样品号）    ' + alms.GetItemData(record.worktypename) + '  进行了监督：' +
        '</td></tr>';
    
   for(var j=0;j<bigitems.length;j++){
      var bigitem = bigitems[j];
      html += '<tr class="infotr"><td class="infoat" colspan=2 align="left" width="10%" style="height:20px">'+bigitem.bigitemname+'</td></tr>';
      for(var i=0;i<samitems.length;i++){
        var samitem = samitems[i];
        if(samitem.bigitemid == bigitem.bigitemid){
           html += '<tr class="infotr"><td class="infoc infoleft" align="left" width="10%" style="height:20px">'+samitem.samitemname+'</td>' +
           '<td class="infoc infoleft"  align="left" width="80%">' +alms.GetItemData(samitem.optionname)+alms.GetItemData(samitem.badbehave)+'</td></tr>';
        }
      }
   }
    html += '<tr class="infotr"><td class="infoat" align="center" width="10%" style="height:'+height+'px">备注</td>' +
    '<td class="infoc infoleft"  align="left" width="90%">' +alms.GetItemData(record.remark)+'</td></tr>';
