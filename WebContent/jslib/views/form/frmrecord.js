Ext.define('alms.frmrecord', {
  extend: 'gpersist.base.busform',
  winpreview: null,
  constructor: function (config) {
    var me = this;
    Ext.apply(config, {
      selectParams: ['BasSample', 'BasParameter', 'FormDirect', 'ValueSource', 'ValueType', 'AlignType'],
      editInfo: '原始记录',
      columnUrl: 'SysSqlColumn.do?sqlid=p_get_frmrecord',
      storeUrl: 'FormSearchFrmRecord.do',
      saveUrl: 'FormSaveFrmRecord.do',
      expUrl: 'FormSearchFrmRecord.do',
      idPrevNext: 'formid',
      hasEdit: true,
      hasDetail: true,
      hasSubmit: false,
      ...
      urlDetail: 'FormGetFrmRecordDetailByForm.do',
      detailTabs: 1,
      hasDateSearch: false,
      hasDetailEdit: true,
      winWidth: 850,
  ...
  OnGetSearchParam: function () {
    ...
    return {
      'fr.formname': tools.GetValueEncode(mep + 'searchformname')
    };
  },
...
//          tools.FormText(tools.MustTitle('表单编号'), 'fr.formid', mep + 'formid', 20, '96%', false, 1),
//          tools.FormCombo(tools.MustTitle('样品'), 'fr.sampleid', mep + 'sampleid', tools.ComboStore('BasSample', null), '96%', true, 5),
          tools.FormText(tools.MustTitle('表单名称'), 'fr.formname', mep + 'formname', 50, '96%', false, 1),
          tools.FormText(tools.MustTitle('长度隔断'), 'fr.formlength', mep + 'formlength', 8, '96%', false, 7, 'isnumber'),
          tools.FormCombo(tools.MustTitle('表单方向'), 'fr.formdirect', mep + 'formdirect', tools.ComboStore('FormDirect', gpersist.SELECT_MUST_VALUE), '96%', false, 9),
          tools.FormText('方法值', 'fr.methodcode', mep + 'methodcode', 20, '96%', true, 11)         
        ]},
        {xtype: 'container', columnWidth: .5, layout: 'anchor', items: [
          tools.FormCombo('检测参数', 'fr.parameterid', mep + 'parameterid', tools.ComboStore('BasParameter', null),'100%', false, 6),
          tools.FormText(tools.MustTitle('宽度隔断'), 'fr.formwidth', mep + 'formwidth', 8, '100%', false, 8, 'isnumber'),
          tools.FormText(tools.MustTitle('分页组数'), 'fr.pagegroup', mep + 'pagegroup', 8, '100%', false, 10, 'isnumber'),
          tools.FormText(tools.MustTitle('分页试件数'), 'fr.pagespec', mep + 'pagespec', 8, '100%', false, 12, 'isnumber'),
          {xtype: 'hiddenfield', name: 'fr.deal.action', id: mep + 'datadeal'},
          {xtype: 'hiddenfield', name: 'fr.createusername', id: mep + 'createusername'},
          {xtype: 'hiddenfield', name: 'fr.modiyserial', id: mep + 'modiyserial'},
          {xtype: 'hiddenfield', name: 'fr.formid', id: mep + 'formid'},
          {xtype: 'hiddenfield', name: 'fr.sampleid', id: mep + 'sampleid'}
        ]}
    ...
    me.disNews = ['formid'];
    me.disEdits = ['formid'];
    me.enNews = ['formname', 'parameterid', 'formlength', 'formwidth', 'formdirect', 'pagegroup', 'pagespec', 'methodcode'];
    me.enEdits = ['formname', 'parameterid', 'formlength', 'formwidth', 'formdirect', 'pagegroup', 'pagespec', 'methodcode'];
    
//    Ext.getCmp(mep + 'sampleid').on('select', me.OnSampleSelect, me);
  },
  
//  OnSampleSelect: function(){
//    var me = this;
//    var mep = me.tranPrefix;
//    
//    var sampleid = Ext.getCmp(mep + 'sampleid').getValue();
//    me.samParStore.load({url:'BasGetListBasSampleParameterDetail.do',params:{sampleid:sampleid}});
//    
//  },
      ...
      {xtype: 'container', anchor: '100%', layout: 'column', items: [
        {xtype: 'container', columnWidth: .33, layout: 'anchor', items: [
          ...
          { xtype: 'fieldcontainer', fieldLabel: '', layout: 'hbox', defaults: { labelStyle: 'font-weight:bold;', margins: '0 15 0 0' }, items: [
            ...
            tools.FormCheck(tools.MustTitle('是否汇总'), 'frdetail.ismutil', mep + 'ismutil', false, 22)]}
        ]},
        {xtype: 'container', columnWidth: .34, layout: 'anchor', items: [
          tools.FormText(tools.MustTitle('项目名称'), 'frdetail.cellname', mep + 'cellname', 200, '96%', false, 2),
        ...
        {xtype: 'container', columnWidth: .33, layout: 'anchor', items: [
          ...
          tools.FormCombo(tools.MustTitle('数据来源'), 'frdetail.valuesource', mep + 'valuesource', tools.ComboStore('ValueSource', gpersist.SELECT_MUST_VALUE), '100%', false, 6),
    ...
    if (me.plDetailGrid && me.plDetailGrid.store) {
      me.plDetailGrid.store.on('beforeload', function (store, options) {      
        Ext.apply(store.proxy.extraParams, {
          'fr.formid': tools.GetValue(mep + 'formid')
        });
      });
    }
...
//    tools.SetValue(mep + 'sampleid', gpersist.SELECT_MUST_VALUE);
    ...
    tools.SetValue(mep + 'formlength', '38');
    tools.SetValue(mep + 'formwidth', '27');
    tools.SetValue(mep + 'pagegroup', '1');
  ...
  OnSetData: function (id, record) {
    ...
    var item = tools.JsonGet(tools.GetUrl('FormGetFrmRecord.do?fr.formid=') + id);
...
//      me.samParStore.load({url:'BasGetListBasSampleParameterDetail.do',params:{sampleid:item.sampleid}});
  ...
  OnPreview: function () {
    ...
    if (!me.winpreview) {
      me.winpreview = tools.GetPopupWindow('alms', 'previewrecord', 
  ...
  OnAfterSave : function(action) {
    ...
    if (action.result && action.result.data) {
      if (me.dataDeal == gpersist.DATA_DEAL_NEW) {
        if (action.result.data.formid) {
          tools.SetValue(mep + 'formid', action.result.data.formid);
        };
      };
    };
    me.OnDetailRefresh();
