Ext.define('alms.editpestgetdetail', {
    ...
    me.editControls = [
      { xtype: 'container', anchor: '100%', layout: 'column', items: [
        { xtype: 'container', columnWidth: .7, layout: 'anchor', items: [
          { xtype: 'container', anchor: '100%', layout: 'column', items: [
            { xtype: 'container', columnWidth: .5, layout: 'anchor', items: [
              ...
              tools.FormText('标准号', 'standcode', mep + 'standcode', 20, '96%', false,26),
              tools.FormText('生产厂家', 'factname', mep + 'factname', 20, '96%', false,26),
              tools.FormText('电话', 'facttele', mep + 'facttele', 20, '96%', false,26),
              tools.FormDate('生产日期', 'prddate', mep + 'prddate', 'Y-m-d', '96%', true, 5,nowdate)
            ]},
            { xtype: 'container', columnWidth: .5, layout: 'anchor', items: [
              tools.FormText('商标', 'trademark', mep + 'trademark', 20, '96%', false,13),
              tools.FormText('样本基数', 'samplebase', mep + 'samplebase', 20, '96%', false,24),
              tools.FormText('准产证号', 'allowcode', mep + 'allowcode', 20, '96%', false,16),
              tools.FormText('登记证号', 'regcode', mep + 'regcode', 20, '96%', false,26),
              tools.FormText('通讯地址', 'factaddr', mep + 'factaddr', 20, '96%', false,26),
              tools.FormText('邮编', 'factpost', mep + 'factpost', 20, '96%', false,16),
              tools.FormText('抽样方式', 'getmethod', mep + 'getmethod', 20, '96%', false,26)
            ]}
          ]},
        ...
        { xtype: 'container', columnWidth: .3, layout: 'anchor', items: [
          { xtype: 'fieldset', id:mep+'gtstand', hidden:false, collapsible: true, title: '规格型号', anchor: '100%', height:240, items: [
//            tools.FormText('规格型号', 'samplestand', mep + 'samplestand', 20, '100%', true,19)
          ]}
        ]}
      ]}
    ];
    ...
    me.disNews = [];
    me.disEdits = [];
    me.enNews = ['sampleid','samplename','samplesize','standcode','factname',
                 'facttele','trademark','samplebase','regcode','factaddr','prddate',
                 'getmethod','samplestand','prdcode','allowcode','factpost','getaddr']; 
    me.enEdits = ['sampleid','samplename','samplesize','standcode','factname',
                  'facttele','trademark','samplebase','regcode','factaddr','prddate',
                  'getmethod','samplestand','prdcode','allowcode','factpost','getaddr'];
    ...
    if (tools.InvalidForm(me.plEdit.getForm())) { 
      ...
      if(tools.GetValue(mep + 'sampleid') == ''){
        ...
        var data = { 
          ...
          standcode: tools.GetValue(mep + 'standcode'),
          ...
          allowcode:tools.GetValue(mep+'allowcode'),
...
//      item.on('select', me.OnStandSelect, me); 
  ...
  OnSetData: function (record,tranid) {
    ...
    tools.SetValue(mep + 'standcode',item.standcode);
    ...
    tools.SetValue(mep + 'allowcode',item.allowcode);
