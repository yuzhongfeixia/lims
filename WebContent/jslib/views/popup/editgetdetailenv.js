Ext.define('alms.editgetdetailenv', {
  ...
  OnBeforeCreateEdit: function () {
    ...
    { xtype: 'container', anchor: '100%', layout: 'column', items: [
      { xtype: 'container', columnWidth: .6, layout: 'anchor', items: [
        { xtype: 'container', anchor: '100%', layout: 'column', items: [
          ...
          { xtype: 'container', columnWidth: .5, layout: 'anchor', items: [
            tools.FormText('样品编号', 'samplecode', mep + 'samplecode', 20, '96%', true,13),
    ...
    me.disNews = [];
    me.disEdits = [];
    me.enNews = ['samplename','sampleid','samplecount','getaddr','samplecode','gpsnum','map']; 
    me.enEdits = ['samplename','sampleid','samplecount','getaddr','samplecode','gpsnum','map'];
    ...
    if (tools.InvalidForm(me.plEdit.getForm())) { 
      ...
      if(tools.GetValue(mep + 'sampleid') == ''){
        ...
        var data = { 
          ...
          samplecode:tools.GetValue(mep + 'samplecode'),
   ...
   searchMap:function(){
    ...
    showInfo:function(e){
      ...
      ensureLocation = function (lng,lat){
          stopMaker();
          var lngs=formatDegree(lng);
    	  var lats=formatDegree(lat);
          tools.SetValue('gpsnum',lngs+','+lats);
          tools.alert("添加成功");
      }
      stopMaker = function (){
...
//        var point = new BMap.Point(116.404,39.915);//定义一个中心点坐标
        var point = new BMap.Point(118.744683,32.063993);//定义一个中心点坐标
