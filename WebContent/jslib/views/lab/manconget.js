Ext.define('alms.manconget', {
  ...
  constructor: function (config) {
    ...
    Ext.apply(config, {
      ...
      saveUrl: 'LabSaveBusGetCon.do',
      expUrl: 'LabSearchBusGet.do',
      urlDetailColumn: 'SysSqlColumn.do?sqlid=p_get_consgetdetail',
  ...
  OnBeforeCreateEdit: function () {
    ...
    var getnotice = Ext.create('Ext.ux.GridPicker', {
      ...
      storeUrl: 'LabSearchBusGetNoticeForGet.do?bgn.gettype=02',
      editable:false,
    ...
    me.editControls = [      
      ...
      { xtype: 'container', anchor: '100%', layout: 'column', items: [
        { xtype: 'container', columnWidth: .25, layout: 'anchor', items: [
          ...
          tools.FormCombo('是否送样', 'bg.sendtype', mep + 'sendtype', tools.ComboStore('SendType', gpersist.SELECT_MUST_VALUE), '96%', false, null,90)
        ]},
        { xtype: 'container', columnWidth: .25, layout: 'anchor', items: [
          tools.FormText('主管单位', 'bg.entermanager', mep + 'entermanager', 20, '96%', true,2,null,90),
          tools.FormText('受委托机构', 'bg.trustdept', mep + 'trustdept', 30, '96%', true,10,null,90),
          tools.FormDate('送样时间', 'bg.senddate', mep + 'senddate', 'Y-m-d', '96%', true, 7,null,90)
        ]},
        { xtype: 'container', columnWidth: .25, layout: 'anchor', items: [
          tools.FormText('受检者姓名', 'bg.testeduser', mep + 'testeduser', 200, '96%', true,3,null,90),
          tools.FormText('抽样人', 'bg.sampleresponname', mep + 'sampleresponname', 30, '96%', true,10,null,90)
        ]},
        { xtype: 'container', columnWidth: .25, layout: 'anchor', items: [
          tools.FormText('产品执行标准', 'bg.prdstand', mep + 'prdstand', 30, '100%', true,10,null,90),
          tools.FormDate('抽样时间', 'bg.getdate', mep + 'getdate', 'Y-m-d', '100%', true, 5,nowdate,90)
        ]}
      ]},
      ...
      {xtype:'hiddenfield',name:'bg.tranidcn',id: mep + 'tranidcn'},
    ...
    me.enNews = ['noticeid','entermanager','tranremark','testeduser','trustdept','prdstand',
                 ...
                 'tasksource','sendtype','sampleresponname','testedname','enterlegal','entertele','enterpost','testtype',
  ...
  OnListNew : function(){
    ...
    me.getdetail = tools.GetPopupWindow('alms', 'editconsgetdetail', 
  ...
  OnLoadData : function(record) {
    ...
    if (item && !Ext.isEmpty(item.tranid)) {
      ...
      tools.SetValue(mep + 'tranidcn', item.tranidcn);
    ...
    if (action.result && action.result.data) {
      if (me.dataDeal == gpersist.DATA_DEAL_NEW) {
        if (action.result.data.tranid) {
          ...
          tools.SetValue(mep + 'tranidcn', action.result.data.tranidcn);
        };
      };
    };
    me.OnDetailRefresh();
  ...
  OnView: function () {
    ...
    me.winpreview.OnSetData('000007',tranid,'02');
  },
