Ext.define('alms.manbusgreen', {
  extend: 'gpersist.base.busform',
	  constructor: function (config) {
	    var me = this;
	    Ext.apply(config, {
	      editinfo: '绿色食品采样单管理',
	      columnUrl: 'SysSqlColumn.do?sqlid=p_get_busgetenv',
	      storeUrl: 'LabSearchBusGet.do',
	      saveUrl: 'LabSaveManBusGreen.do',
	      expUrl: 'LabSearchBusGet.do',
	      urlDetailColumn: 'SysSqlColumn.do?sqlid=p_get_greengetdetail',
	  ...
	  OnBeforeCreateEdit: function () {
	    ...
	    var getnoticeitems = [
	      ' ', { xtype: 'textfield', fieldLabel: '委托编号', labelWidth: 70, width: 200, maxLength: 40, name: 'searchtranid', id: mep + 'searchtranid', allowBlank: true },
	    ...
	    var getnotice = Ext.create('Ext.ux.GridPicker', {
	      fieldLabel: '委托编号', name: 'bg.noticeid', id: mep + 'noticeid', winTitle: '选择委托编号',
	      maxLength: 20, maxLengthText: '受检单位不能超过20字！', selectOnFocus: false, labelWidth: 90,
	      blankText: '委托编号不能为空！', allowBlank: true, anchor: '96%', tabIndex: 1,
	      columnUrl: 'SysSqlColumn.do?sqlid=p_get_getnotice',
	      storeUrl: 'LabSearchBusGet.do?bg.gettype=10&bg.flowstatus=02',
	      editable:false,
	      ...
	      { xtype: 'fieldset', collapsible: true, title: '受检单位', anchor: '100%', items: [  
	        { xtype: 'container', anchor: '100%', layout: 'column', items: [
	          { xtype: 'container', columnWidth: .33, layout: 'anchor', items: [
	            tools.FormText('委托单号', 'bg.tranid', mep + 'tranid', 20, '96%', true,null,null,90),
...
//	            tools.FormCombo('企业性质', 'bg.entertype', mep + 'entertype', tools.ComboStore('EnterType', gpersist.SELECT_MUST_VALUE), '96%', false, null,90),
	            ...
	            tools.FormText('联系人', 'bg.testeduser', mep + 'testeduser', 10, '96%', true,3,null,90),
	            tools.FormText('传真', 'bg.enterfax', mep + 'enterfax', 20, '96%', true,null,null,90)
	            
	          ]},
	          { xtype: 'container', columnWidth: .33, layout: 'anchor', items: [
	            getnotice,
	            tools.FormText('法定代表人', 'bg.enterlegal', mep + 'enterlegal', 200, '96%', true,null,null,90),
//	            tools.FormCombo('企业规模', 'bg.enterscale', mep + 'enterscale', tools.ComboStore('EnterScale', gpersist.SELECT_MUST_VALUE), '96%', false, null,90),
	            tools.FormText('联系人电话', 'bg.testedtele', mep + 'testedtele', 20, '96%', true,3,null,90),
	            tools.FormDate('采样日期', 'bg.getdate', mep + 'getdate', 'Y-m-d', '96%', true, 5,nowdate,90)
	          ]},
	          { xtype: 'container', columnWidth: .34, layout: 'anchor', items: [
                tools.FormText('采样编号', 'bg.tranidcn', mep + 'tranidcn', 20, '96%', true,null,null,90),
                tools.FormText('邮政编码', 'bg.enterpost', mep + 'enterpost', 20, '96%', true,null,null,90),
	            tools.FormText('E-mail', 'bg.testedfax', mep + 'testedfax', 20, '96%', true,3,null,90)
//	            tools.FormDate('采样日期', 'bg.getdate', mep + 'getdate', 'Y-m-d', '96%', true, 5,nowdate,90)
	          ]}
...
//	            tools.FormText('E-mail', 'bg.testemail', mep + 'testemail', 30, '96%', true,null,null,90)
	      ...
	      {xtype: 'hiddenfield',  items: [tools.FormCombo('检验类别', 'bg.testtype', mep + 'testtype', 
	    		  tools.ComboStore('TestType', gpersist.SELECT_MUST_VALUE), '100%', false, null,90), ]},
	    		  
	      {xtype: 'hiddenfield',  items: [tools.FormCombo('企业规模', 'bg.enterscale', mep + 'enterscale',
	    		  tools.ComboStore('EnterScale', gpersist.SELECT_MUST_VALUE),'96%', false, null,90), ]},
          {xtype: 'hiddenfield',  items: [tools.FormCombo('企业性质', 'bg.entertype', mep + 'entertype', 
        		  tools.ComboStore('EnterType', gpersist.SELECT_MUST_VALUE), '96%', false, null,90), ]},
	    ...
	    me.enNews = ['noticeid','entermanager','tranremark','testeduser','trustdept','prdstand',
	                 ...
	                 'testtele','testpost','testaddr','testedname','enterlegal','entertele','enterpost','testtype',
	                  'entertype','enterscale','enteraddr','tranidcn','gettype','sampleusername','enterfax'];
	  ...
	  OnInitData : function() {
	    ...
	    tools.SetValue(mep + 'testfax', '86229784');
	    tools.SetValue(mep + 'testlink', '孙钰洁');
//	    tools.SetValue(mep + 'testemail', 'wanglei@jsagri.gov.cn');
	    tools.SetValue(mep + 'testtele', '86263553');
...
//	      '-',  { id: mep + 'btnCommit', text: '提交', iconCls: 'icon-deal', handler: me.OnSubmit, scope: me },
	  ...
	  OnListNew : function(){
	    ...
	    me.getdetail = tools.GetPopupWindow('alms', 'editgreengetdetail', 
	  ...
	  OnDetailSave: function (e, data) {
	    ...
	    record.data.samplerule = data.samplerule;
	    record.data.keepwarm = data.keepwarm;
	    record.data.factlegal = data.factlegal;
	    record.data.samplesign = data.samplesign;
	    record.data.factemail = data.factemail;
	    record.data.keeptime = data.keeptime;
	    record.data.packmethod = data.packmethod;
	    record.data.samplemethod = data.samplemethod;
	    record.data.getPosition = data.getPosition;
	    record.data.similarvariety = data.similarvariety;
	  ...
	  OnLoadData : function(record) {
	    ...
	    if (item && !Ext.isEmpty(item.tranid)) {
	      ...
	      tools.SetValue(mep + 'tranidcn', item.clientsign);
...
//	      tools.SetValue(mep + 'testemail', item.testemail);
	  ...
	  OnView: function () {
	    ...
	    me.winpreview.OnSetData('000011',tranid,'11');
	  },
	  ...
	  OnBeforeSave:function(){
	    ...
	    var noticed=tools.GetValueEncode(mep+'noticeid');
	    tools.SetValue(mep+'gettype',11);
...
//	    
//	    if(Ext.isEmpty(noticed)){
//		      tools.alert('委托编号不能为空，需选择！');
//		      return false;
//		    }
