Ext.define('alms.manbusenv', {
  ...
  constructor: function (config) {
    ...
    Ext.apply(config, {
      ...
      saveUrl: 'LabSaveManBusEnv.do',
      expUrl: 'LabSearchBusGet.do',
      urlDetailColumn: 'SysSqlColumn.do?sqlid=p_get_envgetdetail',
  ...
  OnBeforeCreateEdit: function () {
    ...
    var getnotice = Ext.create('Ext.ux.GridPicker', {
      fieldLabel: '委托书编号', name: 'bg.noticeid', id: mep + 'noticeid', winTitle: '选择委托书',
      maxLength: 20, maxLengthText: '受检单位不能超过20字！', selectOnFocus: false, labelWidth: 90,
      blankText: '委托书编号不能为空！', allowBlank: true, anchor: '96%', tabIndex: 1,
    ...
    me.editControls = [      
      { xtype: 'container', anchor: '100%', layout: 'column', items: [
        { xtype: 'container', columnWidth: .33, layout: 'anchor', items: [
          tools.FormText('业务编号', 'bg.tranid', mep + 'tranid', 20, '96%', true,null,null,90),
        ...
        { xtype: 'container', columnWidth: .33, layout: 'anchor', items: [
          ...
          tools.FormCombo('企业性质', 'bg.entertype', mep + 'entertype', tools.ComboStore('EnterType', gpersist.SELECT_MUST_VALUE), '96%', true, null,90)
          
          
        ]},
        { xtype: 'container', columnWidth: .34, layout: 'anchor', items: [
          tools.FormText('采样编号', 'bg.tranidcn', mep + 'tranidcn', 20, '100%', true,null,null,90),                                                              
          tools.FormText('联系电话', 'bg.entertele', mep + 'entertele', 20, '100%', true,null,null,90),
      ...
      { xtype: 'container', anchor: '100%', layout: 'column', items: [
        { xtype: 'container', columnWidth: .25, layout: 'anchor', items: [
          ...
          tools.FormText('种植品种', 'bg.planvar', mep + 'planvar', 10, '96%', true,3,null,90)
        ]},
        { xtype: 'container', columnWidth: .25, layout: 'anchor', items: [
          tools.FormDate('采样日期', 'bg.getdate', mep + 'getdate', 'Y-m-d', '96%', true, 5,nowdate,90),
          tools.FormText('采样深度', 'bg.sampledeep', mep + 'sampledeep', 10, '96%', true,3,null,90)
        ]},
        { xtype: 'container', columnWidth: .25, layout: 'anchor', items: [
          tools.FormText('基地名称', 'bg.basename', mep + 'basename', 100, '96%', true,3,null,90),
          tools.FormText('受检者', 'bg.testeduser', mep + 'testeduser', 200, '96%', true,3,null,90)
        ]},
        { xtype: 'container', columnWidth: .25, layout: 'anchor', items: [
          tools.FormText('基地面积', 'bg.basearea', mep + 'basearea', 10, '100%', true,3,null,90)
        ]}
      ]},
    ...
    me.enNews = ['noticeid','entermanager','tranremark','testeduser','trustdept','prdstand',
                 ...
                 'tasksource','sendtype','planvar','testedname','enterlegal','tranidcn', 'entertele','enterpost','testtype',
...
//      '-',  { id: mep + 'btnCommit', text: '提交', iconCls: 'icon-deal', handler: me.OnSubmit, scope: me },
  ...
  OnListNew : function(){
    ...
    me.getdetail = tools.GetPopupWindow('alms', 'editgetdetailenv', 
      {mid:me.mid, tranPrefix:me.tranPrefix, childPrefix: 'egd', dataDeal: gpersist.DATA_DEAL_NEW})
    me.getdetail.on('formlast', me.OnDetailSave, me);
    me.getdetail.OnFormLoad();
    me.getdetail.OnLoadMap();
  ...
  OnListSelect: function(e, record, item, index) {
    ...
    me.getdetail = tools.GetPopupWindow('alms', 'editgetdetailenv', 
      {mid:me.mid, tranPrefix:me.tranPrefix, childPrefix: 'egd', dataDeal: me.dataDeal})
    me.getdetail.on('formlast', me.OnDetailSave, me);
    me.getdetail.OnFormLoad();
    
    me.getdetail.OnLoadMap();
    me.getdetail.OnListSelectMap(record);
  ...
  OnDetailSave: function (e, data) {
    ...
    record.data.samplecode = data.samplecode;
    record.data.gpsnum = data.gpsnum;
  ...
  OnView: function () {
    ...
    me.winpreview.OnSetData('000012',tranid,'12');
  },
  ...
  OnBeforeSave:function(){
    ...
    if(count==0){
      ...
      return false;
    	...
    	tools.alert('样品明细不能大于9个！请复制抽样单再进行添加');
        return false;
    }
