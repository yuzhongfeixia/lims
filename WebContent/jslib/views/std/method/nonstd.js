...
//      winHeight: 450,
  ...
  OnBeforeCreateEdit: function () {
    ...
    var propuser = tools.UserPicker(mep + 'propuser','stdnonstd.propuser','提出人');
    ...
    me.editControls = [
      { xtype: 'container', anchor: '100%', layout: 'column', items: [
        { xtype: 'container', columnWidth: .5, layout: 'anchor', items: [
          tools.FormText('业务编号', 'stdnonstd.tranid', mep + 'tranid', 20, '96%', false, 1),
          propuser,
          tools.FormDate('提议日期', 'stdnonstd.propdate', mep + 'propdate', 'Y-m-d', '96%', false, 5)
        ]},
        { xtype: 'container', columnWidth: .5, layout: 'anchor', items: [
          tools.FormText('检测方法名称', 'stdnonstd.testmethname', mep + 'testmethname', 30, '100%', false, 2,'',90),
          tools.FormText('提出人姓名', 'stdnonstd.propusername', mep + 'propusername', 30, '100%', false, 2,'',90),
          {xtype:'hiddenfield',name:'stdnonstd.deal.action',id: mep + 'datadeal'}
        ]}
      ]},
      tools.FormTextArea('方法来源', 'stdnonstd.methsource', mep + 'methsource', 100, '100%', true, 9, 4),
      tools.FormTextArea('理由', 'stdnonstd.reason', mep + 'reason', 100, '100%', true, 9, 4),
      tools.FormTextArea('试验记录', 'stdnonstd.trialrecord', mep + 'trialrecord', 100, '100%', true, 9, 4),
      tools.FormTextArea('备注', 'stdnonstd.remark', mep + 'remark', 100, '100%', true, 9, 4)
    ];
    me.disNews = ['tranid','propusername'];
    me.disEdits = ['tranid','propusername'];
    me.enNews = [ 'testmethname', 'methsource', 'reason', 'trialrecord', 'remark','propuser','propdate'];
    me.enEdits = [ 'testmethname', 'methsource', 'reason', 'trialrecord', 'remark','propuser','propdate'];
  ...
  OnLoadData: function (item) {
    ...
    if(item && !Ext.isEmpty(item.tranid)){
      var record = tools.JsonGet(tools.GetUrl('StdGetStdNonstd.do?stdnonstd.tranid=') + item.tranid);
      tools.SetValue(mep + 'tranid', item.tranid);
      tools.SetValue(mep + 'testmethname', item.testmethname);
      tools.SetValue(mep + 'methsource', item.methsource);
