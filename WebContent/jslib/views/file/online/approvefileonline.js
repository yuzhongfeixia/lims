Ext.define('alms.approvefileonline', {
  ...
  constructor: function (config) {
    ...
    Ext.apply(config, {
      ...
      storeUrl: 'IncFileSearchIncFileOnline.do',
      saveUrl: 'IncFileApproveIncFileOnline.do',
      expUrl: 'IncFileSearchIncFileOnline.do',
      hasPage: true,
      hasPrevNext: true,
      idPrevNext: 'tranid',
      hasGridSelect: true
    ...
    if (me.gridStore) {
      me.gridStore.on('beforeload', function (store, options) {      
        Ext.apply(store.proxy.extraParams, {
          'ifo.filename': tools.GetEncode(tools.GetValue(mep + 'searchfilename')),
          'ifo.flowstatus': '30'  // 30 审核通过
        });
      });
    }
  },
  ...
  OnBack: function(){
    ...
    var record = tools.OnGridLoad(me.plGrid, '请选择需要驳回的文件申请！');
    ...
    if(record){
      Ext.Msg.confirm(gpersist.CONFIRM_TITLE, '确定驳回申请', function(btn) {
        if (btn == 'yes') {
            Ext.Ajax.request({
                url: 'IncFileDisapproveIncFileOnline.do',
                params: {'ifo.tranid':record.tranid},
                async: false,
                method: 'POST',
  ...
  OnNew:function(){
    ...
    var record = tools.OnGridLoad(me.plGrid, '请选择需要授权的文件！');
  ...
  OnCreateEditWin: function(){
    ...
    if (!me.plAuthPwd) {
      me.plAuthPwd = Ext.create('Ext.form.Panel', {
        ...
        {
          xtype:'hiddenfield',name:'ifo.tranid',id: mep + 'tranid'
        }],
        listeners: {
