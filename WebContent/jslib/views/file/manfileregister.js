Ext.define('alms.manfileregister', {
  ...
  constructor: function (config) {
    ...
    Ext.apply(config, {
      ...
      storeUrl: 'IncFileSearchIncFileRegister.do',
      saveUrl: 'IncFileSaveIncFileRegister.do',
      expUrl: 'IncFileSearchIncFileRegister.do',
      hasPage: true,
      hasPrevNext: true,
      idPrevNext: 'tranid',
      hasGridSelect: true
  ...
  OnBeforeCreateEdit: function () {
    ...
    var applyid = Ext.create('Ext.ux.GridPicker', {
      fieldLabel: '申请编号', name: 'ifr.applyid', id: mep + 'applyid', winTitle: '选择申请编号',
    ...
    me.editControls = [
      { xtype: 'container', anchor: '100%', layout: 'column', items: [
        { xtype: 'container', columnWidth: .5, layout: 'anchor', items: [
          tools.FormText('登记编号', 'ifr.tranid', mep + 'tranid', 30, '96%', false, 1),
          tools.FormText('文件名称', 'ifr.filename', mep + 'filename', 50, '96%', false,3),
          tools.FormText('借阅人', 'ifr.username', mep + 'username', 10, '96%', false,5)
        ]},
        { xtype: 'container', columnWidth: .5, layout: 'anchor', items: [
          applyid,
          tools.FormCombo('借阅类型', 'ifr.borrowtype', mep + 'borrowtype', tools.ComboStore('BorrowType', gpersist.SELECT_MUST_VALUE), '100%', false, 4),
          {xtype:'hiddenfield',name:'ifr.deal.action',id: mep + 'datadeal'},
          {xtype:'hiddenfield',name:'ifr.userid',id: mep + 'userid'},
          {xtype:'hiddenfield',name:'ifr.fileid',id: mep + 'fileid'}
        ]}     
      ]},
      tools.FormTextArea('备注', 'ifr.remark', mep + 'remark', 200, '100%', true,11,3)
    ];
    ...
    if (me.gridStore) {
      me.gridStore.on('beforeload', function (store, options) {      
        Ext.apply(store.proxy.extraParams, {
          'ifr.filename': tools.GetEncode(tools.GetValue(mep + 'searchfilename')),
          'ifr.tranid': tools.GetEncode(tools.GetValue(mep + 'searchuserid'))
        });
      });
    };
  },
