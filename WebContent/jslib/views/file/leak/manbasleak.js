Ext.define('alms.manbasleak', {
  ...
  OnBeforeCreateEdit: function () {
    ...
    var leakuser = tools.UserPicker( mep + 'leakuser','bl.leakuser','泄密人编号',80,'100%');
    leakuser.on('griditemclick', me.OnLeakUserSelect, me);
    
    me.editControls = [
      { xtype: 'container', anchor: '100%', layout: 'column', items: [
        { xtype: 'container', columnWidth: .5, layout: 'anchor', items: [
          tools.FormText('处置编号', 'bl.tranid', mep + 'tranid', 30, '96%', false, 1),
          tools.FormText('泄露人姓名', 'bl.leakusername', mep + 'leakusername', 30, '96%', false, 3),
          tools.FormDate('报告时间', 'bl.reportdate', mep + 'reportdate', 'Y-m-d ', '96%', false, 5,nowdate)
        ]},
        { xtype: 'container', columnWidth: .5, layout: 'anchor', items: [
          leakuser,
          tools.FormDate('泄密时间', 'bl.leakdate', mep + 'leakdate', 'Y-m-d ', '100%', true, 4,nowdate),
          {xtype:'hiddenfield',name:'bl.deal.action',id: mep + 'datadeal'}
        ]}
      ]},
      tools.FormTextArea('泄密原因', 'bl.leakreason', mep + 'leakreason', 100, '100%', true, 9, 5),
      tools.FormTextArea('造成后果', 'bl.leakresult', mep + 'leakresult', 100, '100%', true, 9, 5),
      tools.FormTextArea('泄密简述', 'bl.leakdesc', mep + 'leakdesc', 200, '100%', true, 9, 5)
    ];
