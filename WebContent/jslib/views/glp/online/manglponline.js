...
//      expUrl: 'IncFileSearchIncFileRegister.do',
  ...
  OnBeforeCreateEdit: function () {
    ...
    var applyid = Ext.create('Ext.ux.GridPicker', {
      fieldLabel: '申请编号', name: 'gfo.applyid', id: mep + 'applyid', winTitle: '选择申请编号',
...
//      searchTools:fileitems,
    ...
    me.editControls = [
      { xtype: 'container', anchor: '100%', layout: 'column', items: [
        { xtype: 'container', columnWidth: .5, layout: 'anchor', items: [
          tools.FormText('申请编号', 'gfo.tranid', mep + 'tranid', 30, '96%', false, 1)
        ]},
        { xtype: 'container', columnWidth: .5, layout: 'anchor', items: [
          tools.FormText('文件名称', 'gfo.filename', mep + 'filename', 50, '100%', false,3),
          {xtype:'hiddenfield',name:'gfo.deal.action',id: mep + 'datadeal'},
          {xtype:'hiddenfield',name:'gfo.fileid',id: mep + 'fileid'}
        ]}     
      ]},
      tools.FormTextArea('申请理由', 'gfo.applyreason', mep + 'applyreason', 200, '100%', true,11,8)
    ];
  ...
  OnShow: function () {
    ...
    if(record){
      var file = tools.JsonGet('GlpHasAuthToReadGlp.do',{'gfo.tranuser':gpersist.UserInfo.user.userid,'gfo.fileid':record.fileid});
      if(file && file.fileid){
    ...
    if (me.gridStore) {
      me.gridStore.on('beforeload', function (store, options) {      
        Ext.apply(store.proxy.extraParams, {
          'gf.filename': tools.GetEncode(tools.GetValue(mep + 'searchfilename'))
        });
      });
    };
  },
