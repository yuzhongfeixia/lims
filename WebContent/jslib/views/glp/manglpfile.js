Ext.define('alms.manglpfile', {
  ...
  constructor: function (config) {
    ...
    Ext.apply(config, {
      ...
      saveUrl: 'GlpSaveGlpFile.do',
      saveAllUrl:'GlpSaveAllGlpFile.do',
      expUrl: 'GlpSearchGlpFile.do',
      deleteUrl: 'GlpDeleteGlpFile.do',
      hasGridSelect: true,
      hasPage: true,
      hasPrevNext: true,
      idPrevNext: 'fileid'
    ...
    me.editControls = [
      { xtype: 'container', anchor: '100%', layout: 'column', items: [
        { xtype: 'container', columnWidth: .5, layout: 'anchor', items: [
          tools.FormText('文件编号', 'gf.fileid', mep + 'mffileid', 20, '96%', false, 1),
          tools.FormCombo('文件类别', 'gf.filecate', mep + 'mffilecate', tools.ComboStore('FileCate', gpersist.SELECT_MUST_VALUE), '96%', false, 6),
          tools.FormCombo('文件状态', 'gf.filestatus', mep + 'mffilestatus', tools.ComboStore('FileStatus', gpersist.SELECT_MUST_VALUE), '96%', false, 6),
          tools.FormDate('上传时间', 'gf.uploaddate', mep + 'mfuploaddate', 'Y-m-d H:i', '96%', false, 4)
        ]},
        { xtype: 'container', columnWidth: .5, layout: 'anchor', items: [
          tools.FormText('文件名称', 'gf.filename', mep + 'mffilename', 50, '96%', false, 4),
          tools.FormCombo('文件类型', 'gf.filetype', mep + 'mffiletype', tools.ComboStore('FileType', gpersist.SELECT_MUST_VALUE), '96%', false, 5),
          tools.FormDate('文件时间', 'gf.filedate', mep + 'mffiledate', 'Y-m-d H:i', '96%', false, 4),
          {xtype:'hiddenfield',name:'gf.deal.action',id: mep + 'datadeal'}
        ]}
      ]},
      tools.FormTextArea('文件地址', 'gf.fileurl', mep + 'mffileurl', 50, '98%', true, 9, 3)
    ];
    ...
    if (me.gridStore) {
      me.gridStore.on('beforeload', function(store, options) {
        Ext.apply(store.proxy.extraParams, {
          'gf.fileid' : tools.GetValueEncode(mep + 'searchfileid'),
          'gf.filename' : tools.GetValueEncode(mep + 'searchfilename')
        });
      });
    }
  },
...
//附件上传
  ...
  OnBeforeCreateDetailEdit: function () {
    ...
    me.editDetailControls = [
      { xtype: 'container', anchor: '100%', layout: 'column', items: [
        { xtype: 'container', columnWidth: .5, layout: 'anchor', items: [
//          tools.FormText('文件编号', 'glp.fileid', mep + 'fileid', 30, '96%', false, 12),
          tools.FormCombo('文件类别', 'gf.filecate', mep + 'filecate', tools.ComboStore('FileCate', gpersist.SELECT_MUST_VALUE), '96%', false, 1),
           tools.FormCombo('文件状态', 'gf.filestatus', mep + 'filestatus', tools.ComboStore('FileStatus', gpersist.SELECT_MUST_VALUE), '96%', false, 2)
        ]},
        { xtype: 'container', columnWidth: .5, layout: 'anchor', items: [
          tools.FormCombo('文件类型', 'gf.filetype', mep + 'filetype', tools.ComboStore('FileType', gpersist.SELECT_MUST_VALUE), '96%', false, 1)
        ]}                                                                 
      ]},
      { xtype: 'hiddenfield', name: 'gf.filename', id: mep + 'filename' },
      { xtype: 'hiddenfield', name: 'gf.fileurl', id: mep + 'fileurl' },
      {xtype:'hiddenfield',name:'gf.deal.action',id: mep + 'mdatadeal'}
